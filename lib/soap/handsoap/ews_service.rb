  ...
  module EWS
    module SOAP
      class ExchangeWebService < Handsoap::Service
        ...
        def set_http_options(option_hash)
          if @http_options.nil?
        ...
        def on_create_document(doc)
          ...
          header.add("#{NS_EWS_TYPES}:RequestServerVersion") do |rsv|
        ...
        def on_response_document(doc)
          ...
          doc.add_namespace NS_EWS_TYPES, "#{MS_BASE}/types"
          doc.add_namespace NS_EWS_MESSAGES, "#{MS_BASE}/messages"
        end
        ...
        def on_after_create_http_request(req)
          begin
            ...
            raise EwsLoginError, "Missing credentials."
          end
        end
        ...
        #   ResolveNames
        # @see http://msdn.microsoft.com/en-us/library/aa581054.aspx
        #   UnresolvedEntry
        #
        # @param [String] name an unresolved entry
        # @param [Boolean] full_contact_data whether or not to return full
        #   contact info
        # @param [Hash] opts optional parameters to this method
        # @option opts [String] :search_scope where to seach for this entry,
        #   one of
        #     SOAP::Contacts, SOAP::ActiveDirectory,
        #     SOAP::ActiveDirectoryContacts (default),
        #     SOAP::ContactsActiveDirectory
        # @option opts [String, FolderId] :parent_folder_id either the name
        #   of a folder or it's numerical ID.
        ...
        #
        # @param [String] dist_email The e-mail address associated with the
        #   Distribution List
        # @todo Fully support all of the ExpandDL operations. Today it just
        #   supports taking an e-mail address as an argument
        #
        # XXX: jwc 09.13.2011
        ...
        # Return a list containing the names of all folders under the
        # specified parent folder.
        #
        # @param [String,Symbol] root An folder id, either a
        #   DistinguishedFolderId (must me a Symbol) or a FolderId (String).
        #   This is where to start the search from. Usually
        #   :root,:msgfolderroot,:publicfoldersroot
        # @return [Array<String>] Return an Array of folder names.
        # @raise [EwsError] raised when the backend SOAP method returns an
        #   error.
        def all_folders(root = :msgfolderroot)
          find_folder([:msgfolderroot], 'Shallow')
        end
        ...
        # @param [Hash] opts optional parameters to this method
        def find_folder(parent_folder_ids = [:root], traversal = 'Shallow',
                        folder_shape = {:base_shape => 'Default'}, opts = {})
          ...
          folders = []
          parse!(resp).items.each do |item|
            folders.push build_folder item
          end
        ...
        def get_folder(folder_ids, folder_shape = {:base_shape => 'Default'},
          ...
          folder_ids = [folder_ids] unless folder_ids.is_a? Array
        ...
        # @param [String,Symbol] root An folder id, either a
        #   DistinguishedFolderId (must me a Symbol) or a FolderId (String).
        #   This is where to start the search from. Usually :root,
        #   :msgfolderroot,:publicfoldersroot
        # @param [String] shape the shape of the object to return
        ...
        # @raise [EwsError] raised when the backend SOAP method returns an
        #   error.
        def get_folder_by_name(name, root = :msgfolderroot,
                               shape = 'Default', opts = {})
          traversal = opts[:traversal] || 'Deep'

          # For now the :field_uRI and :field_uRI_or_constant must be in an
          # Array for Ruby 1.8.7 because Hashes are not positional at insertion
          # until 1.9
          restr = {
            :restriction => {
              :is_equal_to => [
...
=end
        ...
        #      class should probably be read-only.
        #
        # Creates folders, calendar folders, contacts folders, tasks folders,
        # and search folders.
        # @see http://msdn.microsoft.com/en-us/library/aa563574.aspx
        # CreateFolder
        #
        # @param [String,Symbol] parent_folder_id either the name of a folder
        #   or it's numerical ID.  See:
        #   http://msdn.microsoft.com/en-us/library/aa565998.aspx
        # @param [Array,String] folder_name The display name for the new
        #   folder or folders
        #
        def create_folder(parent_folder_id, folder_name)
          ...
          resp = invoke("#{NS_EWS_MESSAGES}:CreateFolder", action) do |root|
            build!(root) do
              ...
              folder_name = [folder_name] unless folder_name.is_a? Array
        ...
        # @see http://msdn.microsoft.com/en-us/library/aa564767.aspx
        # DeleteFolder
        #
        # @param [Array,String,Symbol] folder_id either the name of a folder
        #   or its numerical ID. See:
        #   http://msdn.microsoft.com/en-us/library/aa565998.aspx
        # @param [String,nil] delete_type Type of delete to do:
        ...
        def delete_folder(folder_id, delete_type = 'MoveToDeletedItems')
          ...
          resp = invoke("#{NS_EWS_MESSAGES}:DeleteFolder", action) do |root|
            build!(root) do
              ...
              folder_id = [folder_id] unless folder_id.is_a? Array
...
=begin
  XXX:  jwc 09.13.2011
        This doesn't appear to do anything (build_update_folder! isn't
        defined anywhere).

        def update_folder
          action = "#{SOAP_ACTION_PREFIX}/UpdateFolder"
          resp = invoke("#{NS_EWS_MESSAGES}:UpdateFolder", action) do |folder|
            build_update_folder!(folder)
          end
...
=begin
  XXX:  jwc 09.13.2011
        This doesn't appear to do anything (build_move_folder! isn't
        defined anywhere).

        def move_folder
          action = "#{SOAP_ACTION_PREFIX}/MoveFolder"
          resp = invoke("#{NS_EWS_MESSAGES}:MoveFolder", action) do |folder|
            build_move_folder!(folder)
          end
...
=begin
  XXX:  jwc 09.13.2011
        This doesn't appear to do anything (build_copy_folder! isn't
        defined anywhere).

        def copy_folder
          action = "#{SOAP_ACTION_PREFIX}/CopyFolder"
          resp = invoke("#{NS_EWS_MESSAGES}:CopyFolder", action) do |folder|
            build_copy_folder!(folder)
          end
...
=end
        ...
        # XXX: jwc 09.13.2011
        #      Not covered in unit tests and will likely be removed, as we
        #      should never subscribe to a user's notifications (should we?).
        #
        # Used to subscribe client applications to either push or pull
        # notifications.
        # @see http://msdn.microsoft.com/en-us/library/aa566188.aspx Subscribe 
        #
        # @param [Array] folder_ids An Array of folder ids, either a
        #   DistinguishedFolderId (must me a Symbol) or a FolderId (String)
        # @param [Array] event_types An Array of EventTypes that we should
        #   track. Available types are, CopiedEvent, CreatedEvent,
        #   DeletedEvent, ModifiedEvent, MovedEvent, NewMailEvent,
        #   FreeBusyChangedEvent
        # @param [Integer] timeout The number of minutes in which the
        #   subscription will timeout after not receiving a get_events
        #   operation.
        # @todo Decide how/if to handle the optional SubscribeToAllFolders
        #   attribute of the PullSubscriptionRequest element.
        ...
        # notifications.
        # @see http://msdn.microsoft.com/en-us/library/aa566188.aspx Subscribe
        def push_subscribe(folder_ids, event_types, url, watermark=nil,
                           status_frequency=5)
          action = "#{SOAP_ACTION_PREFIX}/Subscribe"
          resp = invoke("#{NS_EWS_MESSAGES}:Subscribe", action) do |root|
            build!(root) do
              push_subscription_request!(folder_ids, event_types, url,
                                         watermark, status_frequency)
            end
          end
        ...
        # @see http://msdn.microsoft.com/en-us/library/aa566199.aspx GetEvents
        #
        ...
        #   See: http://msdn.microsoft.com/en-us/library/aa565261.aspx
        # @param [Hash] opts optional parameters to this method
        def sync_folder_items(folder_id, sync_state = nil, max_changes = 256,
        ...
        # @param [Hash] item_shape defines the ItemShape node
        #   See: http://msdn.microsoft.com/en-us/library/aa565261.aspx
        # @option item_shape [String] :base_shape ('Default')
        #   IdOnly/Default/AllProperties
        # @option item_shape :additional_properties
        #   See: http://msdn.microsoft.com/en-us/library/aa563810.aspx
        # @param [Hash] opts optional parameters to this method
        ...
        # This is actually a CreateItem operation but they differ for
        # different types of Exchange objects so it is named appropriately
        # here.
        # @see http://msdn.microsoft.com/en-us/library/aa566468.aspx
        #
        # @param [String, Symbol] folder_id The folder to save this message in.
        #   Either a DistinguishedFolderId (must me a Symbol) or a FolderId
        #   (String)
        # @param [Hash, Array] items An array of item Hashes or a single item
        #   Hash. Hash This Hash will eventually be passed to add_hierarchy!
        #   in the builder so it is in that format.  Values should be based on
        #   values found here:
        #   http://msdn.microsoft.com/en-us/library/aa494306.aspx
        # @param [String] message_disposition
        #   "SaveOnly/SendOnly/SendAndSaveCopy"
        #   See: http://msdn.microsoft.com/en-us/library/aa565209.aspx
        def create_message_item(folder_id, items,
        ...
        #
        # @param [String, Symbol] folder_id The folder to create this item in.
        #   Either a DistinguishedFolderId (must me a Symbol) or a FolderId
        #   (String)
        # @param [Hash, Array] items An array of item Hashes or a single item
        #   Hash. This Hash will eventually be passed to add_hierarchy! in the
        #   builder so it is in that format. Values should be based on values
        #   found here: http://msdn.microsoft.com/en-us/library/aa564765.aspx
        # @param [String] send_invites
        #   "SendToNone/SendOnlyToAll/SendToAllAndSaveCopy"
        def create_calendar_item(folder_id, items,
                                 send_invites = 'SendToAllAndSaveCopy')
          action = "#{SOAP_ACTION_PREFIX}/CreateItem"

          resp = invoke("#{NS_EWS_MESSAGES}:CreateItem", action) do |node|
            build!(node) do
              create_item!(folder_id, items, message_disposition=false,
                           send_invites, 'calendar')
            end
          end
        ...
        #
        # @param [String, Symbol] folder_id The folder to save this task in.
        #   Either a DistinguishedFolderId (must me a Symbol) or a FolderId
        #   (String)
        # @param [Hash, Array] items An array of item Hashes or a single item
        #   Hash. Hash values should be based on values found here:
        #   http://msdn.microsoft.com/en-us/library/aa494306.aspx
        #   This Hash will eventually be passed to add_hierarchy! in the
        #   builder so it is in that format.
        # @param [String] message_disposition
        #   "SaveOnly/SendOnly/SendAndSaveCopy"
        #   See: http://msdn.microsoft.com/en-us/library/aa565209.aspx
        def create_task_item(folder_id, items,
                             message_disposition = 'SaveOnly')
          action = "#{SOAP_ACTION_PREFIX}/CreateItem"
        ...
        # This is actually a CreateItem operation but they differ for
        # different types of Exchange objects so it is named appropriately here.
        ...
        # @param [Array] item_ids An Array of item ids
        # @param [String] delete_type Type of deletion:
        #   "HardDelete/SoftDelete/MoveToDeletedItems"
        # @param [String, nil] send_meeting_cancellations
        #   "SendToNone/SendOnlyToAll/SendToAllAndSaveCopy"
        #   This is only applicable to CalendarItems and should be nil
        #   otherwise, which is the default
        # @param [String, nil] affected_task_occurrences
        #   "AllOccurrences/SpecifiedOccurrenceOnly"
        #   This is really only related to tasks and can be nil otherwise,
        #   which is the default.
        def delete_item(item_ids, delete_type,
                        send_meeting_cancellations = nil,
                        affected_task_occurrences = nil)
          action = "#{SOAP_ACTION_PREFIX}/DeleteItem"

          resp = invoke("#{NS_EWS_MESSAGES}:DeleteItem", action) do |root|
            build!(root) do
              root.set_attr('DeleteType', delete_type)
              if send_meeting_cancellations
                root.set_attr('SendMeetingCancellations',
                              send_meeting_cancellations)
              end

              if affected_task_occurrences
                root.set_attr('AffectedTaskOccurrences',
        ...
        #      class should probably be read-only.
        #
        # Used to modify the properties of an existing item in the Exchange
        # store
        # @see http://msdn.microsoft.com/en-us/library/aa581084.aspx
        # @param [Array] item_ids An Array of item ids
        # @param [Hash] changes a Hash of changes to be fed to auto_hierarchy!
        # @param [Hash] opts various attributes to set for this update. See
        #   the Technet docs for more info
        def update_item(item_ids, changes, opts)
          action = "#{SOAP_ACTION_PREFIX}/UpdateItem"
          opts[:message_disposition] ||= 'SaveOnly'
          opts[:conflict_resolution] ||= 'AutoResolve'

          resp = invoke("#{NS_EWS_MESSAGES}:UpdateItem", action) do |root|
            build!(root) do
              root.set_attr('MessageDisposition', opts[:message_disposition])
              root.set_attr('ConflictResolution', opts[:conflict_resolution])

              if opts[:send_meeting_invitations_or_cancellations]
                root.set_attr('SendMeetingInvitationsOrCancellations',
        ...
        # @param [Array<Hash>] item_ids An Array of item ids. These item_ids
        #   should be a Hash of :id and :change_key.
        # @param [Boolean] save_item Save item after sending (Think sent-items)
        # @param [String, Symbol,nil] saved_item_folder The folder to save
        #   this item in. Either a DistinguishedFolderId (must me a Symbol) or
        #   a FolderId (String).  Just leave it blank for the default
        #   :sentitems
          ...
          resp = invoke("#{NS_EWS_MESSAGES}:SendItem", action) do |root|
            build!(root) do
              ...
              if saved_item_folder
                saved_item_folder_id!(root,saved_item_folder)
              end
            end
          end
        ...
        # @param [Array<Hash>] files An Array of Base64 encoded Strings with
        #   an associated name hash format= :name => <name>,
...
=begin
  XXX:  jwc 09.13.2011
        This doesn't appear to do anything (build_delete_attachment! isn't
        defined anywhere).

        def delete_attachment
          action = "#{SOAP_ACTION_PREFIX}/DeleteAttachment"

          resp = invoke("#{NS_EWS_MESSAGES}:DeleteAttachment", action) do |a|
            build_delete_attachment!(a)
          end
...
=end
        ...
        # XXX: jwc 09.13.2011
        #      Not currently covered by unit tests, and should be.
        #
...
=begin
  XXX:  jwc 09.13.2011
        ...
        def create_managed_folder
          ...
          resp = invoke("#{NS_EWS_MESSAGES}:CreateManagedFolder", action) do |f|
            build_create_managed_folder!(f)
          end
...
=end
        ...
        #      class should probably be read-only.
        #
        # Adds one or more delegates to a principal's mailbox and sets
        # specific access permissions.
        # @see http://msdn.microsoft.com/en-us/library/bb856527.aspx
        #
        # @param [String] owner The user that is delegating permissions
        # @param [String] delegate The user that is being given delegate
        #   permission
        # @param [Hash] permissions A hash of permissions that will be
        #   delegated. This Hash will eventually be passed to add_hierarchy!
        #   in the builder so it is in that format.
        ...
        # Provides detailed information about the availability of a set of
        # users, rooms, and resources within a specified time window.
        # @see http://msdn.microsoft.com/en-us/library/aa564001.aspx
        def get_user_availability
          action = "#{SOAP_ACTION_PREFIX}/GetUserAvailability"

          resp = invoke("#{NS_EWS_MESSAGES}:GetUserAvailability", action) do |u|
            build_get_user_availability!(u)
          end
        ...
        # XXX: jwc 09.13.2011
        #      Not covered in unit tests and will likely be removed.
        #
        ...
        def set_user_oof_settings(mailbox, oof_state, ext_audience, dt_start,
                                  dt_end, int_msg, ext_mg)
        ...
        private
        #
        # Create a corresponding folder object for the returned type.
        #
        # XXX: jwc 09.13.2011
        #      There should be a Folder factory...
        def build_folder(item)
          folder_type = item.keys.first  
          folder = nil
          case folder_type
          when :calendar_folder
            folder = CalendarFolder.new(item[folder_type.to_sym])
          when :contacts_folder
            folder = ContactsFolder.new(item[folder_type.to_sym])
          when :folder
            folder = Folder.new(item[folder_type.to_sym])
          when :search_folder
            folder = SearchFolder.new(item[folder_type.to_sym])
          when :tasks_folder
            folder = TasksFolder.new(item[folder_type.to_sym])
          else
            puts "unknown folder type '#{folder_type}'"
          end

          folder.ews = self unless folder.nil?
        ...
        def parse!(response, opts = {})
          ...
          if not response
            raise EwsError,
              "Can't parse an empty response. Please check your endpoint."
          end

          EwsParser.new(response).parse(opts)
        end

        # Override the superclasses' invoke so we can add http_options to
        # each request
